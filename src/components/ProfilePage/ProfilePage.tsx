import { Avatar, Button, Card, Col, Divider, List, Row, Space, Spin } from 'antd';
...
const ProfilePage: React.FC<ProfileProps> = (props: ProfileProps) => {
    ...
    React.useEffect(() => {
        ...
        async function retrieveProfileCertificates() {
            ...
            let certificates = await appStore.retrieveProfileDetails(props.email);
            setAwardee(awardee);
    ...
    const handleImageError = (e) => { e.target.src = "https://image.freepik.com/free-vector/certificate-template-vertical_1284-4551.jpg" }
            ...
            <Col>
                <Row style={{ margin: "5% 8% 0" }}>
                    <Avatar
                        ...
                        size={50}
                    >
                        {awardee?.name.split(" ").slice(0, 2).map((n) => n[0]).join("")}
                    </Avatar>
                ...
                <List style={{ margin: "0 8% 0 8%" }}
                    grid={{ gutter: 16, column: 4 }}
                    dataSource={certificates}
                    renderItem={item => (
                        <List.Item >
                            <Card
                                onClick={() => router.push(`/certificate/${item.certificateId}`)}
                                className={styles.box} hoverable
                                cover={<img
                                    onError={handleImageError}
                                    style={{
                                        backgroundColor: "#f4f5fa", height: "80%", objectFit: "contain",
                                        borderRadius: "10px",
                                        border: "1px solid #ccc", padding: "10%"
                                    }} alt="example" src={item.imageUrl} />}
                            >
                                <Meta
                                    className={styles.meta}
                                    title={<Title level={4} ellipsis={true}>{item.certificateName}</Title>} description={item.orgName} />
                            </Card>
                        </List.Item>
                    )}
                />,
